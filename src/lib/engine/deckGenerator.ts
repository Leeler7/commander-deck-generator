...
// loadUserLists was a client-side hook; stubbed for server-side usage
function loadUserLists(): { id: string; cards: string[] }[] { return []; }
...
function isOwnedRarityExempt(cardName: string, collectionNames: Set<string> | undefined, ignoreOwnedRarity: boolean): boolean {
...
async function generateLands(
  ...
  if (nonBasicTarget > 0 && nonBasicEdhrecLands.length > 0) {
    ...
    const nonBasics = pickFromPrefetched(nonBasicEdhrecLands, landCardMap, nonBasicTarget, usedNames, colorIdentity, bannedCards, maxCardPrice, Infinity, { value: 0 }, maxRarity, maxCmc, budgetTracker, collectionNames, landPenalties.size > 0 ? landPenalties : undefined, currency, new Set(), arenaOnly, collectionStrategy, collectionOwnedPercent, ignoreOwnedBudget, ignoreOwnedRarity);
...
export async function generateDeck(context: GenerationContext): Promise<GeneratedDeck> {
  ...
  console.log(`[DeckGen] Budget settings: deckBudget=${deckBudget}, maxCardPrice=${maxCardPrice}, budgetOption=${budgetOption}, currency=${currency}${ignoreOwnedBudget ? ', ignoring owned for budget' : ''}${ignoreOwnedRarity ? ', ignoring owned for rarity' : ''}`);
  ...
  if (mustIncludeNames.length > 0) {
    ...
    for (const name of mustIncludeNames) {
      ...
      if (collectionStrategy === 'full' && mustIncludeSources.get(name) === 'combo' && notInCollection(name, context.collectionNames)) {
  ...
  if (format === 99) {
    for (const staple of stapleRocks) {
      ...
      if (collectionStrategy === 'full' && notInCollection(staple.name, context.collectionNames)) continue;
      try {
        const card = await getCardByName(staple.name, true);
        // Respect budget, rarity, arena-only constraints
        if (!isOwnedBudgetExempt(staple.name, context.collectionNames, ignoreOwnedBudget) && exceedsMaxPrice(card, maxCardPrice, currency)) continue;
        if (!isOwnedRarityExempt(staple.name, context.collectionNames, ignoreOwnedRarity) && exceedsMaxRarity(card, maxRarity)) continue;
        if (notOnArena(card, arenaOnly)) continue;
        markUsed(card.name);
        categorizeCards([card], categories);
        const cmc = Math.min(Math.floor(card.cmc), 7);
  ...
  if (edhrecData) {
    ...
    const relCurveAnalysis = Object.keys(curveTargets).map(Number).map(cmc => ({
      cmc,
    ...
    for (const c of nonLandForScoring) {
      ...
      const type = ['creature', 'instant', 'sorcery', 'artifact', 'enchantment', 'planeswalker']
        .find(tp => t.includes(tp)) || 'other';
